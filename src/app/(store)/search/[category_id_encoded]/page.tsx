...
type Params = Promise<{ category_id_encoded: string }>;
...
export async function generateMetadata(props: { params: Params }) {
  ...
  const product = await getCategory(+category_id);
  const category_name = product?.name || '';

  const canonical = `${BaseUrl}/search/${category_id_encoded}/${encodeCategoryName(
    category_name
  )}`;
